...
import StandardBmpsCell, { QUERY as STANDARD_BMPS_QUERY } from 'src/components/StandardBmpsCell'
...
const StandardBmpsSettingsPage = () => {
  ...
  const handleAddBmp = async () => {
    await createStandardBmp({ variables: { input: { name, description, isStandard: true, siteId: null } } })
    setName('')
    setDescription('')
  }
    ...
    <div>
      ...
      <div className="mt-4">
        ...
        <input
          ...
          className="border p-2"
        />
        <button onClick={handleAddBmp} className="bg-blue-600 text-white p-2 ml-2">Add BMP</button>
      </div>
      <StandardBmpsCell />
    </div>
  )

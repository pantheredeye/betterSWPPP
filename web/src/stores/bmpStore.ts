...
interface BmpStore {
  ...
  updateBmpResponse: (
    bmpId: number,
    responses: Partial<BmpResponses[number]>
  ) => void
...
const useBmpStore = create<BmpStore>((set, get) => ({
  bmpResponses: {},
  updateBmpResponse: (bmpId, responses) =>
    set((state) => ({
      bmpResponses: {
        ...state.bmpResponses,
        [bmpId]: {
  ...
  getFilteredResponses: () => {
    ...
    return Object.entries(bmpResponses).reduce(
      (acc, [bmpId, responses]) => {
        if (
          Object.keys(responses).some(
            (key) => responses[key] !== '' && responses[key] !== false
          )
        ) {

...
const QuickActionsCard = () => {
  ...
  return (
    <div className="rounded-md bg-white p-6 shadow">
      ...
      {!hasStandardBmps ? (
        <Link to={routes.standardBmpSettings()} className="mb-2 w-full block rounded-md bg-blue-600 px-4 py-2 text-white text-center hover:bg-blue-700">
          Configure Standard BMPs
        </Link>
      ) : (
        <Link to={routes.newSite()} className="mb-2 w-full block rounded-md bg-blue-600 px-4 py-2 text-white text-center hover:bg-blue-700">
          Add Site
        </Link>
      )}
    </div>

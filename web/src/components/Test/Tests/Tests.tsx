import type {
  DeleteTestMutation,
  DeleteTestMutationVariables,
  FindTests,
} from 'types/graphql'

import { Link, routes } from '@redwoodjs/router'
import { useMutation } from '@redwoodjs/web'
import type { TypedDocumentNode } from '@redwoodjs/web'
import { toast } from '@redwoodjs/web/toast'

import { QUERY } from 'src/components/Test/TestsCell'
import { truncate } from 'src/lib/formatters'

const DELETE_TEST_MUTATION: TypedDocumentNode<
  DeleteTestMutation,
  DeleteTestMutationVariables
> = gql`
  mutation DeleteTestMutation($id: Int!) {
    deleteTest(id: $id) {
      id
    }
  }
`

const TestsList = ({ tests }: FindTests) => {
  const [deleteTest] = useMutation(DELETE_TEST_MUTATION, {
    onCompleted: () => {
      toast.success('Test deleted')
    },
    onError: (error) => {
      toast.error(error.message)
    },
    // This refetches the query on the list page. Read more about other ways to
    // update the cache over here:
    // https://www.apollographql.com/docs/react/data/mutations/#making-all-other-cache-updates
    refetchQueries: [{ query: QUERY }],
    awaitRefetchQueries: true,
  })

  const onDeleteClick = (id: DeleteTestMutationVariables['id']) => {
    if (confirm('Are you sure you want to delete test ' + id + '?')) {
      deleteTest({ variables: { id } })
    }
  }

  return (
    <div className="rw-segment rw-table-wrapper-responsive">
      <table className="rw-table">
        <thead>
          <tr>
            <th>Id</th>
            <th>Name</th>
            <th>Type</th>
            <th>&nbsp;</th>
          </tr>
        </thead>
        <tbody>
          {tests.map((test) => (
            <tr key={test.id}>
              <td>{truncate(test.id)}</td>
              <td>{truncate(test.name)}</td>
              <td>{truncate(test.type)}</td>
              <td>
                <nav className="rw-table-actions">
                  <Link
                    to={routes.test({ id: test.id })}
                    title={'Show test ' + test.id + ' detail'}
                    className="rw-button rw-button-small"
                  >
                    Show
                  </Link>
                  <Link
                    to={routes.editTest({ id: test.id })}
                    title={'Edit test ' + test.id}
                    className="rw-button rw-button-small rw-button-blue"
                  >
                    Edit
                  </Link>
                  <button
                    type="button"
                    title={'Delete test ' + test.id}
                    className="rw-button rw-button-small rw-button-red"
                    onClick={() => onDeleteClick(test.id)}
                  >
                    Delete
                  </button>
                </nav>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  )
}

export default TestsList
